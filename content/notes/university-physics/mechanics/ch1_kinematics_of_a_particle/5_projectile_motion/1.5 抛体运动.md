...
若 $v_0$ 固定，当 $\theta = 45\degree$ 时，$X$ 取最大值；当 $\theta = 90\degree$ 时（竖直上抛），$T$ 和 $Y$ 最大。
...
>
> 方向由 **右手螺旋法则** 确定：四指顺着 $<\boldsymbol{a}, \boldsymbol{b}>$ 从 $\boldsymbol{a}$ 到 $\boldsymbol{b}$ 画圆弧，大拇指为 $\boldsymbol{c}$ 的方向。由此可知 $\boldsymbol{c} \perp Span\{\boldsymbol{a}, \boldsymbol{b}\}$.
