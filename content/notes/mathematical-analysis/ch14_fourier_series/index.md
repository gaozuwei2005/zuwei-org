...
- 若 $f(x)$ 可以展开成一致收敛的三角级数，那么 $\displaystyle  \begin{cases} a_n = \frac{1}{\pi} \int_{- \pi}^{\pi} f(x) \cos nx \mathrm dx \\\\ b_n = \frac{1}{\pi} \int_{- \pi}^{\pi} f(x) \sin nx \mathrm dx \end{cases}$，其中 $n \ge 0$（乘 $\cos nx$ 或 $\sin nx$ 之后剩下对 $\cos^2 nx$ 或 $\sin^2 nx$ 的积分等于 $\pi$）
